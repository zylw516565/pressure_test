/**
 * 一些基础公共方法, CommonUtil.h
 * Peter.Zhang 2021.08.19
 */
#ifndef __COMMON_UTIL_H__
#define __COMMON_UTIL_H__

#include <muduo/base/Logging.h>

using std::string;

 // CRC16 码表
static int s_auchCRCHi[] =
{
    
};

static int s_auchCRCLo[] =
{
    
};

inline unsigned short UtilGenCRC16(const char* pszMsg, unsigned int nMsgLen)
{
    int iCrcHi = 0xff;
    int iCrcLo = 0xff;
    int iCrcIndex;

    for (unsigned int i = 0; i < nMsgLen; i++)
    {
        iCrcIndex = (iCrcHi ^ pszMsg[i]) & 0xFF;      //crcHi 位异或 （与上 0x0FF 正数）
        iCrcHi = (iCrcLo ^ s_auchCRCHi[iCrcIndex]);   // crcLo 和 （高位数组中获取的值）位异或  
        iCrcLo = s_auchCRCLo[iCrcIndex];              // 根据 crcIndex 在低位数组中获取值 赋值给 crcLo
    }

    return (unsigned short)(iCrcHi << 8 | iCrcLo);
}

inline string ToHexString(const uint8_t* data, off_t offset, ssize_t size)
{
    if (data == NULL)
    {
        LOG_SYSFATAL << "data is NULL";
    }
    if (offset > size)
    {
        LOG_SYSFATAL << "offset is larger than size";
    }
    char hexchar[3];
    string rest;

    for (auto i = offset; i < size; i++)
    {
        snprintf(hexchar, 3, "%02X", data[i]);
        rest.append(hexchar).append(" ");
    }
    return rest;
}

inline string ToHexStringNoSpace(const uint8_t* data, off_t offset, ssize_t size)
{
    if (data == NULL)
    {
        LOG_SYSFATAL << "data is NULL";
    }
    if (offset > size)
    {
        LOG_SYSFATAL << "offset is larger than size";
    }
    char hexchar[3];
    string rest;

    for (auto i = offset; i < size; i++)
    {
        snprintf(hexchar, 3, "%02X", data[i]);
        rest.append(hexchar);
    }
    return rest;
}

struct ConfigParam
{
    int timeout;
    int threadCount;
    int totalPressureTime; //压测总时长
};

#if defined(WIN32) || defined(WIN64)
#define PDEBUG(fmt,...)    fprintf(stderr, "%s :: %s() %d: DEBUG " fmt " \n", __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#define PERROR(fmt,...)    fprintf(stderr, "%s :: %s() %d: ERROR " fmt " \n", __FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#else
#define PDEBUG(fmt, args...)    fprintf(stderr, "%s :: %s() %d: DEBUG " fmt " \n", __FILE__, __FUNCTION__, __LINE__, ## args)
#define PERROR(fmt, args...)    fprintf(stderr, "%s :: %s() %d: ERROR " fmt " \n", __FILE__, __FUNCTION__, __LINE__, ## args)
#endif

#endif //!__COMMON_UTIL_H__