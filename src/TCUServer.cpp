...
bool TCUServer::init(const char* ip, short port, EventLoop* loop, const char* fileBaseDir/* = "filecache/"*/)
{
    m_strFileBaseDir = fileBaseDir;

    InetAddress addr(ip, port);
    m_server.reset(new TcpServer(loop, addr, "ZYL-TCUServer", TcpServer::kReusePort));
    m_server->setConnectionCallback(std::bind(&TCUServer::onConnected, this, std::placeholders::_1));
    //启动侦听
    m_server->start(6);
...
void TCUServer::uninit()
{
    if (m_server)
        m_server->stop();
}

void TCUServer::onConnected(std::shared_ptr<TcpConnection> conn)
{
    if (conn->connected())
    {
...
void TCUServer::onDisconnected(const std::shared_ptr<TcpConnection>& conn)
{
